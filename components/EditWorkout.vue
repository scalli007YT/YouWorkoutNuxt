<template>
    <v-card flat append-icon="mdi-plus" class="border-dashed rounded-xl pa-1"
        style="border-width: 3px; font-size: 1.5em;" subtitle="No Workout found in library" title="Create New Workout"
        @click="handleDialog">
    </v-card>
    ...
    <v-dialog v-model="dialog">
        <v-card prepend-icon="mdi-dumbbell" append-icon="mdi-dumbbell" title="Create Workout"
            ...
            <v-card-text>
                ...
                <v-row dense>
                    <v-select label="Muscle Group*" :items="['Chest', 'Back', 'Shoulders', 'Arms', 'Core', 'Legs']"
                        variant="solo" v-model="musclegroup" chips multiple>
                    </v-select>
                </v-row>
                <v-row dense>
                    <v-select label="Intensity" :items="['Low', 'Medium', 'High']" variant="solo"
